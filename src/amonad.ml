...
sig
  ...
  val fold_m : ('b -> 'a -> 'b m) -> 'b -> 'a list -> 'b m
end
...
struct
  ...
  let rec transpose xs =
    let hds = sum (lift1 (Acommon.take 1) xs) in
    if null hds then [] else
      hds :: transpose (lift1 (Acommon.drop 1) xs)
end
...
module Writer(M : Monoid.S) = struct
  include Make(struct
      type 'a m = M.t * 'a
...
module WriterT(Mon : Monoid.S)(M : MonadBase) =
struct
  module M = Make(M)
  module W = Writer(Mon)
  type t = Mon.t
